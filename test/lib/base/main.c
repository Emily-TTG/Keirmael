...
enum kml_base_result kml_base_main(void) {
	...
	for(kml_base_size_t i = 0; i < KML_BASE_LENGTH(block_sizes) * 2; ++i) {
		...
		if(i >= KML_BASE_LENGTH(block_sizes)) {
			KML_BASE_LOG("=== Allocator Test; Block Size $Z; Non-Zeroed Regions ===\n", block);
		...
		else KML_BASE_LOG("=== Allocator Test; Block Size $Z; Zeroed Regions ===\n", block);

		KML_BASE_LOG("\t== New Regions; Independent ==\n", block);

		enum kml_base_result result = kml_base_allocator_region_new(0, region_a, a, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new(0, $P<a>, $Z)", region_a, a);
		result = kml_base_allocator_region_new(0, region_b, b, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new(0, $P<b>, $Z)", region_b, b);
		result = kml_base_allocator_region_new(0, region_c, c, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new(0, $P<c>, $Z)", region_c, c);
		result = kml_base_allocator_region_new(0, region_d, d, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new(0, $P<d>, $Z)", region_d, d);
		result = kml_base_allocator_region_new(0, region_e, e, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new(0, $P<e>, $Z)", region_e, e);

		KML_BASE_LOG("\t== New Regions; Chain ==\n", block);

		result = kml_base_allocator_region_new(0, region_a, a, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new($P, $P<a>, $Z) -> $Z blocks", 0, region_a, a, ((struct kml_base_allocator_region*) region_a)->free);
		struct kml_base_allocator_region* head = !result ? region_a : 0;
		result = kml_base_allocator_region_new(head, region_b, b, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new($P, $P<b>, $Z) -> $Z blocks", head, region_b, b, ((struct kml_base_allocator_region*) region_b)->free);
		head = !result ? (head ? head : region_b) : 0;
		result = kml_base_allocator_region_new(head, region_c, c, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new($P, $P<c>, $Z) -> $Z blocks", head, region_c, c, ((struct kml_base_allocator_region*) region_c)->free);
		head = !result ? (head ? head : region_c) : 0;
		result = kml_base_allocator_region_new(head, region_d, d, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new($P, $P<d>, $Z) -> $Z blocks", head, region_d, d, ((struct kml_base_allocator_region*) region_d)->free);
		head = !result ? (head ? head : region_d) : 0;
		result = kml_base_allocator_region_new(head, region_e, e, block);
		KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_region_new($P, $P<e>, $Z) -> $Z blocks", head, region_e, e, ((struct kml_base_allocator_region*) region_e)->free);
		head = !result ? (head ? head : region_e) : 0;

		KML_BASE_LOG("\t== Allocations; Concurrent ==\n", block);
		...
		for(kml_base_size_t j = 0; head && j < KML_BASE_LENGTH(allocations); ++j) {
			...
			KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_block_new($P, $P) -> $P", head, &allocated_block, allocated_block);
			...
			KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_allocation_new($P, $P<$Z>, $Z) -> { .buffer = $P, .index = $Z }", head, &allocations[j], j, allocations[j].size, allocations[j].allocation.buffer, allocations[j].allocation.index);
			...
			if(allocated_block) {
				...
				KML_BASE_LOG_RESULT(result, "\t\tkml_test_base_allocator_test_block($P, $Z)", allocated_block, head->block_size);
				if(result) [[clang::unlikely]] {
					KML_BASE_LOG_RESULT(KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED, "\t\t\tkml_test_base_allocator_test_block($P, $Z) != $R", allocated_block, head->block_size, KML_BASE_RESULT_OK);
					test_result = KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED;
				}

				result = kml_base_allocator_block_delete(head, allocated_block);
				KML_BASE_LOG_RESULT(result, "\t\tkml_base_allocator_block_delete($P, $P)", head, allocated_block);
			}
		}
		...
		for(kml_base_size_t j = 0; head && j < KML_BASE_LENGTH(allocations); ++j) {
			if(!allocations[j].result) {
				...
				KML_BASE_LOG_RESULT(result, "\t\tkml_test_base_allocator_test_allocation($P<$Z>)", &allocations[j], j);

				if(result) [[clang::unlikely]] {
					KML_BASE_LOG_RESULT(KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED, "\t\t\tkml_test_base_allocator_test_allocation($P<$Z>) != $R", &allocations[j], j, KML_BASE_RESULT_OK);
					test_result = KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED;
				}
		...
		KML_BASE_LOG("\t== Allocations; Sequential ==\n", block);
		...
		for(kml_base_size_t j = 0; head && j < KML_BASE_LENGTH(allocations); ++j) {
			...
			if(result && result != allocations[j].result) {
				KML_BASE_LOG_RESULT(KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED, "\t\t\tresult ($R) != allocations[$Z].result ($R)", result, j, allocations[j].result);
				test_result = KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED;
			}
			...
			if(!result) {
				...
				KML_BASE_LOG_RESULT(result, "\t\tkml_test_base_allocator_test_allocation($P<$Z>)", &allocations[j], j);
				if(result) [[clang::unlikely]] {
					KML_BASE_LOG_RESULT(KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED, "\t\tkml_test_base_allocator_test_allocation($P<$Z>) != $R", j, &allocations[j], j, KML_BASE_RESULT_OK);
					test_result = KML_BASE_RESULT_ERROR_UNIT_TEST_FAILED;
				}
